...
import { FaRobot, FaDownload, FaSearch, FaGraduationCap, FaTools, FaRocket } from 'react-icons/fa';
import toast from 'react-hot-toast';
import downloadRoadmapPDF from '../../components/RoadmapPDFExport';
import RoadmapLoading from '../../components/RoadmapLoading';
import MiniTypingGame from './MiniTypingGame';
...
const CareerRoadmapGenerator = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const roadmapText = data.roadmapText || '';
      const finalSkills = data.skills || suggestedSkills[getFinalGoal()]?.join(',') || '';
      ...
      const skillList = finalSkills
        .split(',')
        .map(s => s.trim())
        .filter(Boolean);
  
      setSteps(parsedSteps);
      setSkillSuggestions(skillList);
      setGenerated(true);
    } catch (err) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === 'Enter' && !loading) {
        e.preventDefault();
        const isFormComplete = getFinalGoal().trim() && education && skills.trim();
        if (isFormComplete) {
          handleGenerate();
        } else {
          toast.error('Please fill in all fields before generating roadmap.');
  ...
  }, [goal, customGoal, education, skills, loading]);
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        {generated && (
          <div className="space-y-10" ref={roadmapRef} id="roadmap-content">
            <div className="text-center">
              <h2 className="text-3xl font-bold">🛣️AI-Powered Roadmap</h2>
            ...
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">📘 Skill Learning Links</h2>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {skillSuggestions.map((skill, idx) => (
                <a
                  key={idx}
                  href={`https://www.youtube.com/results?search_query=learn+${encodeURIComponent(skill)}+for+${encodeURIComponent(getFinalGoal())}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block bg-[#1e1e2f] border border-blue-500 rounded-xl p-4 text-blue-300 hover:text-blue-200 hover:border-blue-300 transition text-left shadow"
                >
                  🔗 Learn {skill}
                </a>
              ))}
